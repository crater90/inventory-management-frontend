...
function Modal({ modal, setModal, columns, name }) {
	return (
					...
					<form action="#">
						<div className="grid gap-4 mb-4 sm:grid-cols-2">
							{columns?.map(col => {
								return (
									<div>
										<label for={col} className="block mb-2 text-sm font-medium text-gray-900 capitalize">{col}</label>
										<input type="text" name={col} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" placeholder={`Type ${col}`} />
									</div>
								)
							})}
							{/* <div>
