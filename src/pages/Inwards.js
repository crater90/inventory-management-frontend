...
function Inwards() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const url = `${process.env.REACT_APP_API_URL}/api/transactions/search?itemType=1&itemName=${searchInput}`;
      console.log(url);
      const res = await fetch(url, {
        method: 'GET'
      })
                  ...
                  {data && data.map(row => {
                    return (
                      <tr key={row.transactionId} className="border-b hover:bg-gray-100">
                        ...
                        <td className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap ">{formatDate(row.dateOfSupply)}</td>
                        <td className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap ">{row.checkedBy}</td>
                        {(userDetails.type === 0 || userDetails.type === 1) && (
                          <>
